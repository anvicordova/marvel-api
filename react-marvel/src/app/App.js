...
const App = () => {
  ...
  const getCharacter = () => {
    clear();
    axios.get(`${process.env.REACT_APP_API_URL}/character/${search}`)
    .then((response) => {
      console.log(response);
      const character = response.data.data.attributes
          ...
          <div class="panel story-characters-info">
            ...
            <ul>
              {storyCharacters && storyCharacters.map((item, i) =>{
                  const {id, name, thumbnail_url} = item.attributes
                  return (
                    <li key={id}>
                      <div class="character-info">
                        <h1>{name}</h1>
                        <img src= {thumbnail_url} alt={character.name}/>
                      </div>
                    </li>
                  )
                })
